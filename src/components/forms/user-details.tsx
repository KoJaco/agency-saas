...
// TODO: maybe move global declarations of schema data to another file or encapsulate locally?
...
const UserDetails = ({ id, type, subAccounts, userData }: Props) => {
    ...
    // get auth user details and useEffects
    useEffect(() => {
        // fetch details when data changes
        if (data.user) {
    ...
    // get associated permissions for user's sub account
    useEffect(() => {
        if (!data.user) return;
    ...
    // TODO: is form meant to be included?
    useEffect(() => {
        // Reset data
        if (data.user) {
    ...
    async function onPermissionChange(
        ...
        if (response) {
            ...
            toast({
                variant: "destructive",
                title: "Failed",
                description: `Could not update permission in sub account with id: ${subAccountId}`,
            });
        }
                        ...
                        {authUserData?.role === "AGENCY_OWNER" && (
                            <div>
                                ...
                                <FormDescription className="mb-4">
                                    You can give Sub Account access to team
                                    member by turning on access control for each
                                    Sub Account. This is only visible to agency
                                    owners
                                </FormDescription>
